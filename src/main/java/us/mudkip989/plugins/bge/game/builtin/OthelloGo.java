...
import us.mudkip989.plugins.bge.game.processors.*;
...
public class OthelloGo extends Game {
    ...
    public OthelloGo(Matrix4f transform, World wrld, String options) {
        ...
        //spawn parts
        //base
        ModelDisplay baseboard = new ModelDisplay(TransformUtils.blankTransform().translate(0f, 0.05f, 0f).scale(1, 0.1f, 1), world, uuid, new ItemStack(Material.SANDSTONE));
        baseboard.setParent(frame);
        boardParts.add(baseboard);

        //spawn lines
        for (int i = 0; i < boardSize; i++){
            ModelDisplay line = new ModelDisplay(TransformUtils.blankTransform().translate((1f / boardSize) * (i+0.5f) - 0.5f, 0.101f, 0).scale(0.1f/boardSize, 0.002f, 1), world, uuid, new ItemStack(Material.BLACK_CONCRETE));
            line.setParent(frame);
        }
        for (int i = 0; i < boardSize; i++){
            ModelDisplay line = new ModelDisplay(TransformUtils.blankTransform().translate(0, 0.101f, (1f / boardSize) * (i+0.5f) - 0.5f).scale(1, 0.002f, 0.1f/boardSize), world, uuid, new ItemStack(Material.BLACK_CONCRETE));
            line.setParent(frame);
        }
        ...
        //spawn pieces
//        GroupObject part = new GroupObject(new Matrix4f(), world, uuid);
...
//                BasicButton collider = new BasicButton(new Matrix4f(), world, uuid, new hitbox(new Vector3f(1, 1, 1), new Vector3f()));
